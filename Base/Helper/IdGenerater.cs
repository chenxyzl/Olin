...
public class IdGenerater
{
    ...
    {
        if (node >= _nodeLimit) A.Abort($"node Id must less than {_nodeLimit}", serious: true);
    ...
    {
        if (_ins != null) A.Abort("_ins init repeated");
    ...
    {
        _ins = A.NotNull(_ins, "_ins not init");
        ...
        while (true)
        {
            ...
            if (time >= _timeFlagLimit) A.Abort($"time >= {_timeFlagLimit} value: {time}", serious: true);
