...
public class HttpHotfixManager : Single<HttpHotfixManager>
{
    ...
    public void ReloadHanlder()
    {
        ...
        {
            var attr = A.NotNull(type.GetCustomAttribute<HttpHandlerAttribute>(),
                "HttpHandlerAttribute must exist");
            A.Ensure(!handlesTemp.ContainsKey(attr.Router), $"router{attr.Router} must not repeated");
    ...
    {
        return A.NotNull(handles[router], $"handler:{router} not found");
    }
}